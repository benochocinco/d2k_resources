...
Obviously, we can add more random effects to the model. In the particular example above, we could add both a random slope and intercept at the class level using `(IQ|class)`. We could have multiple levels of hierarchies (e.g., students within classes within schools within districts...) We could also use different distributions or parameterizations for the random effects. The possibilities are endless!
